...
class nataUIFormBasic {
    ...
    constructor( options = {}, dataset, events = {} ) {
        ...
        console.trace( options.form.dataset );
        if ( typeof options.form.dataset !== "undefined")  {
            //self.options.form.dataset
            self.dataset = options.form.dataset;
        }
        else {
            if ( typeof self.dataset == "undefined") self.dataset = {};
            self.dataset.data = [];
        }
        //nata.sessionStorage.setItem( "debug99", self.dataset.data );
        //console.log( nata.sessionStorage.getItem( "debug99" ) );
        ...
        if ( typeof self.dataset.data == "undefined" ) self.dataset.data = [];
    ...
    adapter	( data ) {
        console.trace( "%c nataUIFormBasic.adapter", "background: blue; color: #fff;" );
        ...
        for (i=0; i<data.length; i++) {
            ...
            /* anterior
            if ( data[i].atr == "" ) {
                objData = data[i].response;
            }
            else {
                console.log( objData );
                console.log( data[i] );
                if ( typeof data[i].response !== "undefined") objData[ data[i].atr ] = data[i].response;
            }
            */
    ...
    async buttonNext () {
        ...
        if (self.options.form.idSection == self.options.sections.data.length) {
            ...
            if (typeof self.events.beforeSend == "function") {
                self.events.beforeSend();
            }
    ...
    fieldSave(element, id, value) {
        console.log("%c nataUIFormBasic.fieldSave ", "background:blue;color:white;font-weight:bold;font-size:10px");
        const self = this;
        ...
        for (i = 0; i < self.options.form.fields.length; ++i) {
            if (self.options.form.fields[i].se.toString().trim().toLowerCase() == "check") {
                ...
                if (self.options.form.fields[i].c == id) {
                    //console.log(self.options.form.fields[i].tr.toString().trim());

                    if (self.options.form.fields[i].tr.toString().trim().toLowerCase() == "tension-arterial") {
                        self.options.form.fields[i].response = {};
    ...
    fieldSaveOptionButton(element) {
        console.log("%c nataUIFormBasic.fieldSaveOptionButton ", "background:blue;color:white;font-weight:bold;font-size:10px");
        const self = this;
    ...
    //@audit deprecar
    loadData() {
        console.trace( "nataUIFormBasic.loadData" );
        const self = this;
        nata.sessionStorage.setItem( "debug101", self.dataset.data );
        console.log( nata.sessionStorage.getItem( "debug101" ) );

        if ( typeof self.dataset == "undefined") return false;
        if ( Array.isArray( self.dataset.data ) ) {
            if ( self.dataset.data.length == 1) {
                self.dataset.data = self.dataset.data[0];
            }
            else {
                if ( self.dataset.data.length > 0 ) self.dataset.data = self.adapterDataInput();
            }
        }

        if ( Object.keys(self.dataset.data).length > 0 ) {
            let i, prop, arrResponse;
            console.log(self.options.form.fields);
            for (i = 0; i < self.options.form.fields.length; ++i) {

                if (self.options.form.fields[i].atr.toString().trim() == "" && self.options.form.fields[i].se.toString().trim().toLowerCase() != "radio") {
                    continue;
                }

                if (self.options.form.fields[i].se.toString().trim() == "radio") {
                    for (prop in self.dataset.data) {
                        if (Object.prototype.hasOwnProperty.call(self.dataset.data, prop)) {
                            if (typeof self.options.form.fields[i].response == "undefined") self.options.form.fields[i].response = {};
                            self.options.form.fields[i].response[prop] = self.dataset.data[prop];
                        }
                    }
                }
                else {
                    for (prop in self.dataset.data) {
                        ...
                        if (Object.prototype.hasOwnProperty.call(self.dataset.data, prop)) {
                            //console.log(self.options.form.fields[i].dc, prop, self.options.form.fields[i].dc == prop);
                            //console.log( self.options.form.fields[i].atr, prop, self.options.form.fields[i].atr == prop );
                            if (self.options.form.fields[i].atr == prop) {
                                if (self.options.form.fields[i].tr == "divipola") {
                                    //console.log(self.dataset.data, prop);
                                    // la divipola puede llegar null
                                    if (self.dataset.data[prop] !== null) {

                                        if (typeof self.dataset.data[prop] == "object") {
                                            self.options.form.fields[i].response = {
                                                d: self.dataset.data[prop].d,
                                                m: self.dataset.data[prop].m
                                            };
                                        }
                                        else {
                                            arrResponse = self.dataset.data[prop].toString().split("-");
                                            self.options.form.fields[i].response = {
                                                d: arrResponse[0],
                                                m: arrResponse[1]
                                            };
                                    ...
                                    self.options.form.fields[i].response = self.dataset.data[prop];
                                }
                ...
                if (self.options.form.fields[i].response === null) self.options.form.fields[i].response = "";
            }
            console.log(self.options.form.fields);
        }
    }
    ...
    renderLayout() {
        console.log("%c nataUIFormBasic.renderLayout ", "background:blue;color:white;font-weight:bold;font-size:10px");
        // eslint-disable-next-line no-unused-vars
        const self = this;
    ...
    render() {
        console.trace("%c nataUIForm.render ", "background:blue;color:white;font-weight:bold;font-size:10px");
        ...
        if (  self.options.config.index.adapter ==  "mysql-nosql") {
            self.loadDataAdapter();
        }
        else {
            //console.log ( self.dataset.data );
            if ( self.dataset === "undefined" ) self.dataset = {};
            if ( self.dataset.data === "undefined" ) self.dataset.data = [];

            self.oper = "insertar";
            if ( ( Array.isArray( self.dataset.data ) && self.dataset.data.length == 1 )
                || typeof self.dataset.data == "object" ) {

                nata.sessionStorage.setItem( "debug100", self.dataset.data );
                console.log( nata.sessionStorage.getItem( "debug100" ) );


                self.oper = "editar";
                self.loadData();
                //#endregion cargar respuestas
            }
        }
        ...
        for (i = 0; i < elements.length; ++i) {
            elements[i].addEventListener("click", function () {
                ...
                self.fieldSaveOptionButton(this);
    ...
    validate() {
        console.log("%c nataUIFormBasic.validate ", "background:blue;color:white;font-weight:bold;font-size:10px");
        const self = this;
    ...
    validateFormRadio () {
        console.log("%c nataUIFormBasic.validateFormRadio ", "background:blue;color:white;font-weight:bold;font-size:10px");
        const self = this;
