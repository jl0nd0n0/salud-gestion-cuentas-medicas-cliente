/* globals nata, doT, Clusterize, session, $, localforage */
...
class nataUIList {
        ...
        if (data.length > 0) {
            rows = data.map(function (record) {
                ...
                return {
                    dataset: record,
                    values: tmp,
                    markup: doT.template(document.getElementById(self.config.templateRender).innerHTML)(record).toString().trim(),
                    active: true
                };
            });
        }
    ...
    render() {
        ...
        setTimeout(function() {
            ...
            self.timestamp = dialogTimestamp;

            let options = Object.assign({}, self.dialog);
            if (typeof self.dialog.callback == "object") {
                options.callback = self.dialog.callback;
            }
            console.log(options);
            options.id = dialogTimestamp;
            alert("paso 01");
            options.body = {
                html: nata.ui.template.get("templateLayoutList-1", { id: self.index, search: true})
            };
			...
			*/
            ...
            options.title = self.dialog.title + `
				<span id="count-${self.index}" class="badge rounded-pill bg-white text-primary ms-2">0</span>
			`;
            options.toolbar = {
                close: {
                    boolButton: true,
            ...
            options.callbacks = self.callbacks;
            console.log(options);
